    ...
    <div class="row">
        <div class="col-md-12 col-sm-12">
            <div class="card card-box">
                <div class="card-head">
                    <header>Ubah Harga Barang</header>
                </div>
                <div class="card-body " id="bar-parent2">
                    <div class="row">
                        <div class="col-md-12 col-sm-12">
                            <form action="{{ route('admin.harga_barang.update', $data->id) }}" method="POST"
                                ...
                                <div class="mb-3">
                                    ...
                                    <select class="form-control @error('barang_id') is-invalid @enderror" name="barang_id"
                                        aria-describedby="barangHelp" id="selectbarang" required>
                                        <option value="">-- Pilih --</option>
                                        @foreach ($barang as $br)                                            
                                            <option value="{{ $br->id }}" {{ $data->id_barang == $br->id ? 'selected' : '' }}>{{ $br->nama }}</option>
                                        @endforeach
                                    </select>
                                    @error('barang_id')
                                ...
                                <div class="mb-3">
                                    ...
                                    <select class="form-control @error('jenis_outlet_id') is-invalid @enderror"
                                        ...
                                        @foreach ($jenis_outlet as $jo)                                            
                                            <option value="{{ $jo->id }}" {{ $data->id_jenis_outlet == $jo->id ? 'selected' : '' }}>{{ $jo->nama }}</option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="mb-3">
                                    ...
                                    <input type="number" class="form-control @error('harga') is-invalid @enderror"
                                        ...
                                        placeholder="Masukkan Harga Barang" autocomplete="off" value="{{ $data->harga }}"
                                        name="harga" required>
                                    @error('harga')
